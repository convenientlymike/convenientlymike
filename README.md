...
| 🅢🎮 **Stacktionary — a reconstructed word game** *(private · [case study](https://github.com/convenientlymike/case-studies/blob/main/16-blocktionary-recovery-and-rebuild.md))* | Reverse-engineered a **dead-media** online word game (the classic MyLeague / Case's Ladder *Blocktionary*) from its surviving rules — defeating a Cloudflare WAF with the **coherent-browser transport** (a real Chrome, not a cleverer `curl`), and **proving the game itself was gone by logging in** — then **rebuilt it honestly**. A pure-TS **canonical rules engine** is the single source of truth for the UI, an adaptive bot, and **24 forcing-function + self-play tests**; it reproduces the verified mechanics exactly (timed turns, the rising pile, the **6+ Challenge/bluff → ×3 / −50 / carry**), with the five unrecoverable constants **honesty-banded** as tunable design behind one config. Full 2026 wrapper: **Expo iOS/Android/Web**, rank/XP/achievements meta on local-first storage, theme engine, applied accessibility + i18n (en/es), and a **flag-gated (OFF) monetization** layer. | `TypeScript` · `React Native` · `Expo` · `zustand` · `Chrome-for-Testing` |
...
| 🪤 **[android-inline-hook-mapping-trap](https://github.com/convenientlymike/android-inline-hook-mapping-trap)** *(open source)* | Why a byte-perfect Android/Linux `arm64` inline hook fires **0×** — a shared library is mapped *twice* (raw file `mmap` vs the dynamic-linker ELF `LOAD`), so the naive *"lowest mapping + rva"* resolver patches the copy the CPU never runs. Ships a **tested `/proc/maps` resolver** that returns the *executed* address (largest-`r-xp` `.text` → ELF load base) + the field-guide methodology: the diagnostic signature, the cross-thread coherency toolkit, and the meta-lesson (*when every hypothesis in a class fails, attack the shared assumption*). | `Python` · `Reverse engineering` · `ARM64` · `Android` |
...
| 🔎 **Design-grade multi-domain audit framework** *(methodology · [case study](https://github.com/convenientlymike/case-studies/blob/main/17-design-grade-audit-framework.md))* | The repeatable loop behind an "audit everything" sweep, distilled into an invokable tool: **map → find (per axis) → adversarially *refute* each finding → synthesize a ranked, honesty-banded backlog**, then ship each confirmed fix as a verified increment **paired with a forcing function that bites**. The rigor is in the failure modes it bakes in — a *"0 findings"* is a hypothesis (read the recon maps), a *"this is handled"* needs a **higher** bar than a positive finding (a false-*safe* is the worst error), and the audit's **own** recommendations get refuted too. Run across six subsystems, it drove a latent isolation-bug class from **17 sites to 0** behind a ratcheting lint. | `Multi-agent` · `Adversarial verification` · `Forcing functions` · `Honesty-banding` |
...
| 🕸️ **[corpuscrawl](https://github.com/convenientlymike/corpuscrawl)** *(open source · [case study](https://github.com/convenientlymike/case-studies/blob/main/18-universal-corpus-scraper.md))* | Turn any wikis/guide sites into an **offline, full-text-searchable corpus** — a **tiered fetch engine** (MediaWiki API → polite static HTTP → **real-browser escalation** on a JS/Cloudflare wall, *verified against live Cloudflare*) with pluggable adapters and resumable **SQLite + FTS5** packs. Ships a correct, self-contained **robots.txt matcher** (the stdlib parser false-negatives an explicitly-*allowed* path). CLI + library; 19 tests, CI on 3.10–3.12. | `Python` · `httpx` · `Playwright` · `SQLite FTS5` |
...
| 🛡️ **[`lan-recon`](https://github.com/convenientlymike/lan-recon)** *(open source · [▶ live demo](https://convenientlymike.github.io/lan-recon/) · [case study](https://github.com/convenientlymike/case-studies/blob/main/15-network-security-assessment.md))* | Read-only, authorization-gated LAN discovery + asset inventory — fingerprints every host by service/banner/mDNS/vendor and emits an **evidence-graded** inventory (`VERIFIED` / `INFERRED` / `UNKNOWN` — never a guess dressed as a fact). **Refuses to run until scope is affirmed** (a forcing function, negative-control-tested in CI) and **never sends a state-changing packet**. Ships the four-phase assessment methodology behind it. | `Bash` · `Network security` · `CLI` · `Read-only` |
...
![Network security](https://img.shields.io/badge/Network_security-16A34A?style=flat-square&logo=wireshark&logoColor=white)
